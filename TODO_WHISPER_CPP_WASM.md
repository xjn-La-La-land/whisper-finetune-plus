...
✅ 已完成 @ 2026-06-05（产物部署在 P1-1 已做，本次补 MIME + 隔离头）
   ...
   **待部署验证（de-risk 闸门）**：生产走 Cloudflare 具名隧道，需实测隧道是否透传这两个头。
   浏览器控制台一行即可判断：`crossOriginIsolated` 应为 `true`。若被隧道剥掉 → 退路是
   客户端 coi-serviceworker 注入，或编一个单线程 WASM 兜底（性能 -30%，见风险登记）。
...
✅ 文档完成 @ 2026-06-06（⏳ 生产 COOP/COEP 透传待用户 sync 后实测）
   ...
   **遗留（非文档、是运行时闸门）**：生产 Cloudflare 隧道是否透传 COOP/COEP 仍需用户把代码
   sync 到服务器后，在 https://app.carespeechai.cn 控制台敲 `crossOriginIsolated` 实测确认。
   这也是风险登记表里那条「隧道不透传 COEP/COOP」的最终验收点。
