...
add_arg("model_path",  type=str, default="/home/featurize/whisper-large-v3-finetuned", help="合并模型的路径，或者是huggingface上模型的名称")
        ...
        with torch.autocast(device_type="cuda"):
            with torch.no_grad():
                ...
                for pred, label in zip(decoded_preds, decoded_labels):
                    tqdm.write(f"预测 (Pred) : {pred}")
                    tqdm.write(f"标签 (Label): {label}")
                    tqdm.write("-" * 50)
